...
| 电话控制 | 已实现（信令） | 可拨号、接听、拒接和挂断，并保留最近 100 条运行期记录；拨号和挂断已真机验证，UAC 双向音频仍为实验功能 |
...
- 电话拨号和挂断已通过大疆模块真机验证；来电接听和异常场景仍需完整回归。
- 模块 UAC 探测、显式启用和浏览器本地音频桥为实验功能，双向音频尚待真机验收；启用 USB Audio 可能需要重启模块。
- 若 `USBCFG` 只有 6 个 USB 功能参数，模块固件不支持通过 AT 启用 UAC；DJOneHub 会停止写入，不能仅靠软件配置解决。
- 已测试的 Baiwang QDC507 固件 `QDC507GLEFM21_01.001.02.004` 会拒绝 `QPCMV?`、UAC `QPCMV=1,2` 和串口 PCM `QPCMV=1,0`，因此只支持电话信令，不能使用浏览器双向通话音频。
- 不要向 QDC507 直接刷入标准 Quectel EG25 固件；该固件基线并非 Quectel 官方开发，错误刷写可能导致模块无法启动。
- 流量统计仅供参考，不等同于运营商账单。
