...
| 多模块管理 | 预览 | 通过 USB Hub 同时发现和管理多个模块，短信、来电、eSIM 与 AT 状态按模块隔离 |
...
连接多个模块时，页面顶部会出现模块选择器。程序使用模块 IMEI 生成本机 Device ID，并以 USB Hub 端口链跟踪当前连接；可以为每台模块设置本机别名。为避免 macOS 默认路由和模块私网网段冲突，当前版本只允许一台模块被设置为上网模式，其他模块仍可保持短信/管理模式。
...
在“短信”页面展开“Bark 通知转发”，填写包含 `{message}` 占位符的 API 链接和自定义别名，然后启用并保存。例如：
...
长回复使用跨 Realtime audio delta 保持状态的流式 FIR 降采样器，上行 8→16 kHz 插值也跨媒体消息保持连续，不会在云端分块边界重新初始化。Swift 播放队列采用读指针推进，并使用约 200 ms 首包缓冲和欠载后重新蓄水；UAC 满载时不会反复复制或重排整段 PCM。播放结束由独立的 `audio.done` 事件确认，不再受 usage 事件到达时机影响；默认提示 Agent 将单轮语音控制在约 20 秒内，复杂内容分段播报。

可靠性与安全默认值：

- 可分别配置主 Provider、MiniMax STT 降级和跨 Provider 降级；失败后使用指数退避熔断。
- 本地音频宿主断线后自动重连，来电方重新说话时会清空未播放的 AI 音频。
- 电话状态查询和挂断工具可自动执行；DTMF 必须由网页操作员在 45 秒内确认。
